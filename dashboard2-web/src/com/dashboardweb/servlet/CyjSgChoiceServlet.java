...
@WebServlet(urlPatterns = {"/cyj/sg-choice"})
public class CyjSgChoiceServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String path = String.format("http://127.0.0.1:5000/cyj/choice?gu=%s&category=%s&storecnt=%s",
		...
		if (status == 200) {
			...
			CyjSgChoiceVO[] sgchoiceList = gson.fromJson(reader, CyjSgChoiceVO[].class);
			
			req.setAttribute("sg-choice-list", sgchoiceList);
	...
	// 브라우저에 응답
	RequestDispatcher rd = req.getRequestDispatcher("/WEB-INF/views/cyj-sg-choice-list.jsp");
	rd.forward(req, resp);
