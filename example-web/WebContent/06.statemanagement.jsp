...
Cookie cookie = new Cookie("cookiedata", //쿠키의 이름
			   				encoded); // 쿠키의 값
...
//세션에 값을 저장
session.setAttribute("sessiondata", //세션에 저장한 데이터 이름
					 Integer.valueOf(sessionData + 1));


//3. Application (ServletContext)
// 생명주기 : 서버 시작할 때 생성 - 서버가 종료할 때 소멸
// 모든 브라우저가 공유하는 데이터 저장소
if (application.getAttribute("applicationdata") != null) {
	applicationData = 
		(Integer)application.getAttribute("applicationdata"); 
}

application.setAttribute("applicationdata", //애플리케이션 저장되는 데이터
						 Integer.valueOf(applicationData + 1));
